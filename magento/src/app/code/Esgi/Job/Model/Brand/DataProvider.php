...
namespace Esgi\Job\Model\Brand;
...
{
    /**
     * @var \Esgi\Job\Model\ResourceModel\Brand\Collection
     */
    ...
    public function getData()
    {
        ...
        /** @var \Esgi\Job\Model\Brand $brand */
        foreach ($items as $brand) {
            $this->loadedData[$brand->getId()] = $brand->getData();
        }

        $data = $this->dataPersistor->get('job_brand');

        if (!empty($data)) {
            $brand = $this->collection->getNewEmptyItem();
            $brand->setData($data);
            $this->loadedData[$brand->getId()] = $brand->getData();
            $this->dataPersistor->clear('job_brand');
        }
